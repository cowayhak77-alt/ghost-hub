...
[제목 작성법 - 강력한 후킹 필수!]
반드시 아래 8가지 중 1개:
1. "{keyword} 이거 모르면 {{금액}}원 날립니다"
2. "알 사람만 아는 {keyword} 진실"
3. "{keyword} 지금만 {{혜택}}, 내일부터 인상"
4. "{keyword} 샀다가 멘붕 왔습니다"
5. "업계인이 폭로하는 {keyword} 진실"
6. "{keyword} vs {{경쟁품}}, 결과 충격"
7. "{keyword} {{기대}}했는데 {{반전}}"
8. "{keyword} 지금 확인 안 하면 {{위험}}"
            ...
            with st.spinner('페르소나 선택 중...'):
                try:
                    ...
                    if json_match:
                        ...
                        cta1_html = f'<div style="margin: 30px 0; padding: 20px; border: 3px solid #000; border-radius: 5px;"><p style="font-size: 15px; color: #000; margin: 0 0 10px 0; font-weight: bold;">{hook1}</p><p style="font-size: 16px; color: #000; margin: 0; font-weight: bold;">👉 {product} 최저가 & 혜택 확인하기</p></div>'
                        
                        cta2_html = f'<div style="margin: 30px 0; padding: 20px; border: 3px solid #000; border-radius: 5px;"><p style="font-size: 15px; color: #000; margin: 0 0 10px 0; font-weight: bold;">{hook2}</p><p style="font-size: 16px; color: #000; margin: 0; font-weight: bold;">👉 {product} 지금 바로 구매하기</p></div>'
            ...
            with st.spinner('전문가 페르소나 접속 중...'):
                try:
                    ...
                    if json_match:
                        ...
                        # Unsplash 이미지 삽입
                        images = get_unsplash_images(keyword, 5)
                        if images:
...
def render_tistory_info():
    ...
    copy_mode = st.radio("복사 모드", ["미리보기 복사", "HTML 복사"], key="tistory_info_mode", horizontal=True)
    ...
    if st.session_state.tistory_info_display:
        ...
        st.components.v1.html(f"""
            <button onclick="copyRich()" style="width:100%; padding:20px; background:#FF6B35; color:white; border:none; border-radius:12px; font-weight:bold; cursor:pointer; font-size:18px;">
                🟠 {'미리보기' if copy_mode == '미리보기 복사' else 'HTML'} 복사하기
            </button>
            <script>
            function copyRich() {{
                const html = `{html_code}`;
...
[절대 준수]
1. 자기소개 금지
2. 제목: {product_name} 포함, 20자 내외, 강력한 후킹
   예: "{product_name} 지금 안 사면 50만원 더 냅니다"
3. 첫 줄부터 팩트로 공격
4. 5개 소제목 (<h3>)
5. 중간 [CTA_1], 끝 [CTA_2]
6. 이미지 금지
...
def render_tistory_profit():
    ...
    copy_mode = st.radio("복사 모드", ["미리보기 복사", "HTML 복사"], key="tp_mode", horizontal=True)
...
"""
    ...
    if st.session_state.tistory_profit_content:
        ...
        if copy_mode == "미리보기 복사":
            ...
            st.subheader("📋 HTML 코드")
            st.text_area("HTML", value=st.session_state.tistory_profit_content, height=500)
        
        safe = st.session_state.tistory_profit_content.replace("`", "\\`").replace("$", "\\$")
        
        st.components.v1.html(f"""
            <button onclick="copy()" style="width:100%; padding:20px; background:#FF1493; color:white; border:none; border-radius:12px; font-weight:bold; cursor:pointer; font-size:18px;">
                🟠 {'미리보기' if copy_mode == '미리보기 복사' else 'HTML'} 복사하기
            </button>
            <script>
            function copy() {{
